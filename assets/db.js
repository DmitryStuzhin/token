/* ═══════════════════════════════════════════════════════════════════
   ...
   1. Справочники (ref) — предметы с сеткой экзамена, темы и банк задач.
      Задаются в коде, одинаковы для всех. Банк генерируется: условия
      и ответы считаются, поэтому автопроверка работает по-настоящему.

   2. Состояние (state) — пользователи, привязки, группы, занятия,
      задания и попытки. Изначально ПУСТО: всё появляется только после
      того, как кто-то зарегистрируется и начнёт работать.
      Хранится в localStorage; в бою это таблицы в БД.
   ═══════════════════════════════════════════════════════════════════ */
window.DB = (function () {
  const LS_KEY = 'token.state.v1';
  ...
  /* ── состояние: пусто до первой регистрации ──────────────────── */
  const EMPTY = () => ({
    ...
    lessons: [], lessonAttendance: [], assignments: [], mockExams: [],
    attempts: [], session: null,
  });

  const STATE_KEYS = Object.keys(EMPTY());
  ...
  function load() {
    ...
    const st = EMPTY();
    if (saved) STATE_KEYS.forEach(k => { if (saved[k] !== undefined) st[k] = saved[k]; });
    state = Object.assign(st, { subjects, topics, tasks, taskShape: {
  ...
  function save() {
    if (!state) return;
    const out = {};
    STATE_KEYS.forEach(k => { out[k] = state[k]; });
    /* импортированные задачи храним отдельно от сгенерированных */
    out.importedTasks = state.tasks.filter(t => t.source !== 'generated');
