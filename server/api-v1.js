...
const listQuerySchema = z.object({
  ...
  sort: z
    .enum(['id', 'name', 'title', 'number', 'createdAt', 'startsAt', 'dueAt', 'submittedAt'])
    .default('id'),
...
const bodySchemas = [
  [
    ...
    z.object({
      ...
      consents: z.object({
        personal_data: z.literal('2026-08-13'),
        terms: z.literal('2026-08-13'),
      }),
      ...
      yearsExp: z.coerce.number().min(0).max(80).optional(),
  ...
  [
    /^\/auth\/login\/code$/,
    z.object({ challenge: z.string().min(20).max(500), code: z.string().min(9).max(40) }),
  ],
  ...
  [
    /^\/auth\/password\/reset$/,
    z.object({ token: z.string().min(20).max(500), password: z.string().min(10).max(200) }),
  ],
  [
    /^\/invites$/,
    z.object({
      kind: z.enum(['enrollment', 'group']),
  ...
  [
    /^\/lessons\/[^/]+\/links$/,
    z.object({
      type: z.enum(['call', 'board', 'material']).optional(),
  ...
  [
    ...
    z.object({
      ...
      visibility: z.enum(['private', 'student', 'parent']).optional(),
    }),
  ],
  [/^\/lessons\/[^/]+\/tasks$/, z.object({ taskId: z.string().min(1).max(200) })],
  [
    /^\/lessons\/[^/]+\/status$/,
    z.object({ status: z.enum(['planned', 'done', 'moved', 'cancelled', 'missed']) }),
  ],
  [
    /^\/assignments$/,
    z.object({
  ...
  [
    /^\/attempts\/[^/]+\/review$/,
    z.object({ score: z.number().min(0).max(10), comment: z.string().max(2000).optional() }),
  ],
  [/^\/prefs$/, z.object({ channel: z.string().min(1).max(100), enabled: z.boolean() })],
  [
    /^\/tasks\/import$/,
    z.object({ tasks: z.array(z.record(z.string(), z.unknown())).min(1).max(1000) }),
  ],
];
...
function withProblemDetails(req, res, next) {
  ...
  res.json = (body) => {
    if (res.statusCode >= 400 && body && body.error && !body.type) {
      ...
      return json(
        problem(res.statusCode, titles[res.statusCode] || 'Ошибка', body.error, req, {
...
async function idempotency(req, res, next) {
  ...
  if (key.length < 8 || key.length > 200) {
    return res
      .status(400)
      .json({ error: 'Для этой операции нужен Idempotency-Key длиной 8–200 символов' });
  }
  ...
  if (existing) {
    ...
    return res
      .status(409)
      .set('Retry-After', '1')
      .json({ error: 'Запрос с таким Idempotency-Key ещё выполняется' });
  }
...
router.get('/state.js', (req, res) =>
  res.status(410).json({ error: 'Используйте screen bootstrap API' }),
);
...
router.get('/me', A.requireUser, (req, res) =>
  sendCacheable(req, res, {
...
router.patch(
  ...
  asyncRoute(async (req, res) => {
    ...
    const lesson = await Policy.ownedLesson(req.app.locals.repository, req.tutorId, req.params.id);
    if (!lesson) return res.status(404).json({ error: 'Занятие не найдено' });
    ...
    const updated = await Policy.ownedLesson(req.app.locals.repository, req.tutorId, req.params.id);
  ...
  asyncRoute(async (req, res) => {
    ...
    if (model === 'student-dashboard' && req.user.role === 'student') {
      ...
      const student = String(
        req.query.student || (req.user.role === 'student' ? req.profile.id : ''),
      );
...
router.get(
  ...
  asyncRoute(async (req, res, next) => {
    ...
    if (req.params.resource === 'reviews')
      items = items.filter((item) => item.status === 'submitted');
    const filters = {
      subject: 'subjectId',
      status: 'status',
      student: 'studentId',
    ...
    for (const [queryName, field] of Object.entries(filters)) {
      if (query[queryName])
        items = items.filter((item) => String(item[field] || '') === query[queryName]);
    }
    ...
    if (query.cursor) {
      try {
        offset = Number(JSON.parse(Buffer.from(query.cursor, 'base64url').toString()).offset) || 0;
      } catch {
        return res.status(422).json({ error: 'Некорректный cursor' });
      }
    }
