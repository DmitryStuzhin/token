...
test('email verification is mandatory, expiring and single-use', async () => {
  ...
  assert.equal(
    (await agent.get(verification.pathname + verification.search)).headers.location,
    '/login.html?verified=0',
  );
  assert.equal(
    (await signIn(agent, credentials.email, credentials.password, '/api/v1', '192.0.2.5')).status,
    200,
  );
  ...
  db.prepare(
    `UPDATE account_tokens SET expires_at='2000-01-01T00:00:00.000Z'
    WHERE purpose='verify_email' AND consumed_at IS NULL`,
  ).run();
  const expiredUrl = new URL(expired.body.verificationUrl);
  assert.equal(
    (await request(app).get(expiredUrl.pathname + expiredUrl.search)).headers.location,
    '/login.html?verified=0',
  );
});

test('registration requires current consent versions and stores an immutable receipt', async () => {
  ...
  assert.equal(
    db.prepare('SELECT 1 FROM users WHERE email=?').get('no-consent@example.test'),
    undefined,
  );

  const rows = db
    .prepare(
      `SELECT consent_type,document_version,accepted_at,ip,user_agent
    FROM user_consents uc JOIN users u ON u.id=uc.user_id WHERE u.email=? ORDER BY consent_type`,
    )
    .all(credentials.email);
  assert.deepEqual(
    rows.map((row) => [row.consent_type, row.document_version]),
    [
      ['personal_data', '2026-08-13'],
      ['terms', '2026-08-13'],
    ],
  );
  assert.ok(rows.every((row) => row.accepted_at && row.ip != null && row.user_agent != null));
});
...
test('password reset is non-enumerating, single-use and revokes every session', async () => {
  ...
  assert.equal(
    (await signIn(first, credentials.email, credentials.password, '/api/v1', '192.0.2.11')).status,
    200,
  );
  assert.equal(
    (await signIn(second, credentials.email, credentials.password, '/api/v1', '192.0.2.12')).status,
    200,
  );
...
test('session inventory can revoke one session without exposing cookie tokens', async () => {
  ...
  assert.equal(
    (await signIn(first, credentials.email, password, '/api/v1', '192.0.2.21')).status,
    200,
  );
  assert.equal(
    (await signIn(second, credentials.email, password, '/api/v1', '192.0.2.22')).status,
    200,
  );
...
test('an SMTP outage degrades delivery without breaking signup or leaking accounts', async () => {
  ...
  try {
    ...
    assert.ok(
      db
        .prepare("SELECT 1 FROM security_events WHERE event_type='email_delivery_failed' LIMIT 1")
        .get(),
      'сбой доставки попадает в журнал безопасности',
    );
    ...
    assert.equal(
      (await request(app).get(link.pathname + link.search)).headers.location,
...
test('a wrong code is burned after five attempts', async () => {
  ...
  assert.ok(
    db
      .prepare("SELECT 1 FROM security_events WHERE event_type='code_attempts_exhausted' LIMIT 1")
      .get(),
  );
});
